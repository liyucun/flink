...
/** Handle request for listing uploaded dependency jars. */
public class SchedulerListHandler
    ...
    public SchedulerListHandler(
        ...
        this.schedulerDir = requireNonNull(schedulerDir);
    ...
    protected CompletableFuture<JarListInfo> handleRequest(
        ...
        return CompletableFuture.supplyAsync(
                () -> {
                    try {
                        final File[] list = getSchedulerFiles();
                        ...
                        for (File f : list) {
                            ...
                            if (name.length() < 5 || !name.endsWith(".py")) {
                                continue;
                            }
    ...
    private File[] getSchedulerFiles() {
        if (schedulerDir == null) {
            return EMPTY_FILES_ARRAY;
        }

        final File[] list = schedulerDir.listFiles((dir, name) -> name.endsWith(".py"));
        if (list == null) {
            log.warn(
                    "Scheduler script upload dir {} does not exist, or had been deleted externally. "
